...
for (( i=1 ; i<${#files[@]} ; i++ )); do
	...
	if [[ $npoints != 0 ]]; then
		...
		/usr/bin/ncks -O -d time,-$npoints,-1 ${files[$i-1]} "$TEMP/$(basename ${files[$i-1]}).tmp"
		...
		if [[ $npoints != 1 ]] || [[ ! $comp =~ "True" ]]; then
			if [[ $npoints != 1 ]] && [[ $comp =~ "True" ]]; then
				...
				/usr/bin/ncks -O -d time,0,-2 "$TEMP/$(basename ${files[$i-1]}).tmp" "$TEMP/$(basename ${files[$i-1]}).no_overlap.tmp" # delete and save a tmp file
			...
			/usr/bin/ncrcat -O "$TEMP/$(basename ${files[$i-1]}).tmp" ${files[$i]} "$TEMP/$(basename ${files[$i]}).tmp"
		...
		/usr/bin/ncks -O -d time,0,-$(($npoints+1)) "${files[$i-1]}" "$TEMP/$(basename ${files[$i-1]}).tmp" # delete times moved and save tmp file
	...
	if [[ $npoints != 0 ]]; then
		...
		# Move selected times to a new file
		/usr/bin/ncks -O -d time,0,$(($npoints-1)) ${files[$i]} "$TEMP/$(basename ${files[$i]}).tmp"
		/usr/bin/ncrcat -O ${files[$i-1]} "$TEMP/$(basename ${files[$i]}).tmp" "$TEMP/$(basename ${files[$i-1]}).tmp"
		# Remove selected times from the file
		echo -e "  \033[33m==\033[m Removing $npoints timesteps out of ${files[$i]}..."
		/usr/bin/ncks -O -d time,$npoints,-1 ${files[$i]} "$TEMP/$(basename ${files[$i]}).tmp"
